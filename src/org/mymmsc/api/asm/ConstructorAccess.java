...
public abstract class ConstructorAccess<T> {
	 ...
	 *            The instance of the enclosing type to which this inner
	 *            instance is related to (assigned to its synthetic this$0
	 *            field).
	 */
	...
	static public <T> ConstructorAccess<T> get(Class<T> type) {
		...
		synchronized (loader) {
			try {
				...
				if (!isNonStaticMemberClass) {
					...
					enclosingClassNameInternal = enclosingType.getName()
							.replace('.', '/');
					try {
						type.getDeclaredConstructor(enclosingType); // Inner
																	// classes
																	// should
																	// have
																	// this.
				...
				cw.visit(V1_1, ACC_PUBLIC + ACC_SUPER, accessClassNameInternal,
						null, "org/mymmsc/api/asm/ConstructorAccess", null);

				insertConstructor(cw);
				insertNewInstance(cw, classNameInternal);
				insertNewInstanceInner(cw, classNameInternal,
						enclosingClassNameInternal);
		...
		try {
			ConstructorAccess<T> access = (ConstructorAccess<T>) accessClass
					.newInstance();
			access.isNonStaticMemberClass = isNonStaticMemberClass;
			return access;
		} catch (Exception ex) {
			throw new RuntimeException(
					"Error constructing constructor access class: "
							+ accessClassName, ex);
		}
	}
	...
	static private void insertConstructor(ClassWriter cw) {
		...
		mv.visitMethodInsn(INVOKESPECIAL,
				"org/mymmsc/api/asm/ConstructorAccess", "<init>", "()V");
		mv.visitInsn(RETURN);
		mv.visitMaxs(1, 1);
		mv.visitEnd();
	}
	...
	static void insertNewInstanceInner(ClassWriter cw,
			String classNameInternal, String enclosingClassNameInternal) {
		MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, "newInstance",
		...
		if (enclosingClassNameInternal != null) {
			...
			mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/Object", "getClass",
					"()Ljava/lang/Class;");
			mv.visitInsn(POP);
			mv.visitMethodInsn(INVOKESPECIAL, classNameInternal, "<init>", "(L"
					+ enclosingClassNameInternal + ";)V");
			mv.visitInsn(ARETURN);
			mv.visitMaxs(4, 2);
		} else {
