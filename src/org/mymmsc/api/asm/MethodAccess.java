...
public abstract class MethodAccess {
	...
	public int getIndex(String methodName, Class... paramTypes) {
		for (int i = 0, n = methodNames.length; i < n; i++)
			if (methodNames[i].equals(methodName)
					&& Arrays.equals(paramTypes, parameterTypes[i]))
				return i;
		throw new IllegalArgumentException("Unable to find public method: "
	...
	static public MethodAccess get(Class<?> type) {
		...
		synchronized (loader) {
			try {
				...
				cw.visit(V1_1, ACC_PUBLIC + ACC_SUPER, accessClassNameInternal,
						null, "org/mymmsc/api/asm/MethodAccess", null);
				{
					mv = cw.visitMethod(ACC_PUBLIC, "<init>", "()V", null, null);
					mv.visitCode();
					mv.visitVarInsn(ALOAD, 0);
					mv.visitMethodInsn(INVOKESPECIAL,
							"org/mymmsc/api/asm/MethodAccess", "<init>", "()V");
					mv.visitInsn(RETURN);
					mv.visitMaxs(0, 0);
					mv.visitEnd();
				}
				{
					...
					if (!methods.isEmpty()) {
						...
						mv.visitTableSwitchInsn(0, labels.length - 1,
								defaultLabel, labels);
						...
						for (int i = 0, n = labels.length; i < n; i++) {
							...
							if (i == 0)
								mv.visitFrame(Opcodes.F_APPEND, 1,
										new Object[] { classNameInternal }, 0,
										null);
							else
							...
							buffer.append(')');
							buffer.append(Type.getDescriptor(method
									.getReturnType()));
							mv.visitMethodInsn(INVOKEVIRTUAL,
									classNameInternal, method.getName(),
									buffer.toString());

							switch (Type.getType(method.getReturnType())
									.getSort()) {
							case Type.VOID:
		...
		try {
			...
			throw new RuntimeException(
					"Error constructing method access class: "
							+ accessClassName, ex);
		}
	}
}
